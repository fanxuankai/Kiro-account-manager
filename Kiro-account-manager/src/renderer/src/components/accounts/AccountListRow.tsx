...
function AccountListRowComponent({
  ...
  const handleClearSuspended = useCallback((e: React.MouseEvent) => {
    e.stopPropagation()
    updateAccountStatus(account.id, 'active', undefined)
  }, [account.id, updateAccountStatus])
  ...
  const handleSwitchFree = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  }, [account, isEn, isSwitchingFree, alreadyFreeTier, updateAccount])
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex items-center gap-1.5">
        ...
        {/* 扣款卡（账单快照回写，仅有数据的账号显示；min-w 保持跨行对齐） */}
        {account.subscription.cardLast4 && (
          <Badge
            variant="outline"
            className="text-[10px] h-5 px-1.5 font-normal border-muted-foreground/30 bg-muted/30 text-muted-foreground min-w-[86px] flex items-center justify-center gap-1"
