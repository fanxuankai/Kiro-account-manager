...
const en = {
  ...
  nav: {
    ...
    register: 'Register',
    loginPool: 'GitHub Pool',
    googlePool: 'Google Pool',
  ...
  register: {
    ...
    cfMail: 'CF Mail',
    cfMailBaseURL: 'CF Mail Worker URL',
    cfMailAdminPassword: 'CF Mail admin password',
    cfMailDomain: 'CF Mail domain pool',
    cfMailPrefix: 'CF Mail fixed prefix (optional)',
    cfMailTest: 'Test connectivity',
    cfMailTesting: 'Testing...',
    cfMailDesc: 'CF self-hosted email (dreamhunter2333/cloudflare_temp_email) · admin mode: query mails via GET /admin/mails + x-admin-auth (no JWT/Turnstile); with domain catch-all, any prefix@domain is received; "Test connectivity" verifies worker + admin auth without touching AWS registration',
    emailLabel: 'Email',
