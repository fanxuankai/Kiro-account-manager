...
const zh = {
  ...
  register: {
    ...
    cfMail: 'CF 邮箱',
    cfMailBaseURL: 'CF 邮箱 Worker 地址',
    cfMailAdminPassword: 'CF 邮箱 admin 密码',
    cfMailDomain: 'CF 邮箱自建域名池',
    cfMailPrefix: 'CF 邮箱固定前缀（可选）',
    cfMailTest: '测试连通性',
    cfMailTesting: '测试中...',
    cfMailDesc: 'CF 自建邮箱 (dreamhunter2333/cloudflare_temp_email) · admin 模式：走 GET /admin/mails + x-admin-auth 查邮件，无需 JWT/Turnstile；配合域名 catch-all，任意 prefix@域名 都能收；可点「测试连通性」验证 worker + admin 认证（不碰 AWS 注册接口）',
    emailLabel: '邮箱',
