...
### v1.7.34 (2026-9-14) — 新增账单页：账单快照 + 汇总卡 + 账号管理同款筛选

- **新增（账单页）**: 侧边栏新增「账单」页 — 顶部四张汇总卡（下期总扣款 / 将续费 / 已排期 Free / 未检查）随筛选实时重算；筛选与账号管理页同款交互：分组下拉互斥单选（全部/未分组/各分组，带计数与颜色圆点）、搜索框（邮箱/昵称）、彩色计划 chip（KIRO FREE/PRO/PRO+/PRO MAX/POWER，配色与账号页一致）、下期状态 chip、标签彩色 chip、邮箱域名后缀 chip（计数、超 16 个折叠）；宽表格显示计划单价、计费周期、本周期应收、下期账单金额@日期（红=将扣款、青=$0 已排期 Free）、扣款卡（品牌+末四位，悬浮显示有效期）、最近发票（点击直达 Stripe 官方收据页）；列表顺序与账号管理页共用同一排序设置（默认最近使用优先）；「检查选中 / 检查全部账单」走只读门户链路（并发 2）回写快照，行内可单独刷新
- **新增（账单快照，零额外请求）**: 「检查续费 / 切 Free」（含批量「全部切 Free」）的 GET subscriptions 响应本就内联计划单价、计费周期、本周期与下期账单金额、扣款卡、最近发票及收据链接 — 此前解析几个 ID 后即丢弃，现在随同一响应回写到 `subscription`（金额为分、时间为毫秒）；GET 追加 `expand=data.default_payment_method` 让卡面信息（品牌/末四位/有效期）内联返回，请求数不变（仍每账号 4 个）
- **改进（窗口）**: 应用启动直接最大化（显示前先 maximize，不闪小窗）
- **范围**: 从未订阅的纯 Free 账号无账单，不出现在账单页；已排期切 Free（含网页手动切）的账号可在账单页检查补快照；发票历史（完整列表）本期未做，行内只展示最近一张

### v1.7.33 (2026-9-14) — 修复刷新悬浮球被全局样式压到左下角

- **修复**: v1.7.32 的悬浮球实际渲染在左下角且被窗口裁切 — 应用根容器的 `.ambient-bg > * { position: relative }`（未分层自定义 CSS）会覆盖子元素的 Tailwind `fixed` 类，悬浮球退化为文档流末尾的 flex 子项并被 `right` 偏移推出屏幕；改用 `createPortal` 挂到 body（与 UpdateDialog/CloseConfirmDialog 同一做法），右下角定位恢复正常；悬浮球同步放大到 80px（环 5px、中心 14px 字号），球下方加类型小标签「刷新 Token / 刷新用量」（自动触发追加「· 自动」）

### v1.7.32 (2026-9-14) — 刷新进度改为右下角悬浮环形进度球

- **改进**: 批量刷新进度从标题栏下方全局横条改为右下角 44px 悬浮环形进度球 — 中心显示 n/N 进度数字，不再带文字横条、不遮挡内容，也不占用页面布局空间；置于顶层所有页面可见，`pointer-events-none` 鼠标完全穿透不挡点击；空闲时自动隐藏

---

### v1.7.31 (2026-9-13) — 批量订阅选择账号过滤已获取链接

- **改进**: 批量订阅「获取链接」页的选择账号列表不再显示已获取过链接（成功或获取中）的账号 — 副标题计数、全选、「获取链接 (N)」按钮同步按待获取集合计算；失败/过期条目不算已获取，对应账号仍可勾选重试；删掉链接后账号重新出现
- **改进**: 「获取链接」改为与已有链接合并非整表替换 — 分批获取时不再冲掉之前已拿到的链接（原来每次获取都会用本批次覆盖整个链接列表），条目更新按账号定位；获取完剩余账号时已有链接的勾选状态保留

---

### v1.7.30 (2026-9-11) — 修复订阅管理列表空白 + 加载计划按钮常可用

- **修复**: 订阅管理 tab 账号行数 ≥50 时列表整体空白 — 虚拟滚动容器 `max-h` 靠内容撑高与 `contain:strict`（尺寸包含忽略子元素高度）组合导致容器高度为 0、一行不渲染；改为固定高度 `h-[60vh]`，行数不足 50 的直接渲染分支不受影响
- **改进**: 批量订阅「加载计划」按钮不再因无可升级账号而置灰 — 计划列表与账号是否可升级无关，无可升级账号时自动改用任一有凭证的账号加载

---

### v1.7.29 (2026-9-11) — 账号管理直切 Free + GitHub 无痕登录入口收敛（含 1.7.27/1.7.28 未发布改动）

- **新增**: 账号管理卡片/列表行「切 Free」按钮（绿色下箭头）— 无需进订阅管理页，单账号一键降级到免费套餐（Stripe 门户链路，无需打开浏览器）；已是 Free 或已排期降级的账号自动禁用
- **改进**: 删除工具栏独立「注册」按钮，改为「添加账号 → 在线登录 → GitHub 账号（无痕模式）」— 固定无痕窗口登录，不再依赖隐私模式开关
- **改进**: 批量刷新进度从右下角悬浮小卡改为标题栏下方全局横条，所有页面可见；自动刷新（定时器触发）同样显示并标注「自动」
- **改进**: 扫码支付二维码放大（显示 260px → 320px，位图按 2 倍生成，Retina 屏不再发糊），对话框同步加宽
- （v1.7.28，随本版一并发布）官网直登私密浏览器、自动刷新双链路与进度反馈、网络层提速重试
- （v1.7.27，随本版一并发布）闲置账号库 — 不保活账号的物理隔离库，与账号管理双向移动

---

### v1.7.26 (2026-9-4) — 在线更新链路验证版本

- 无功能变更；用于验证 v1.7.25 引入的 mac 自研在线更新（从 1.7.25「关于 → 检查更新 → 在线更新」应可无感升到本版本）

---

### v1.7.25 (2026-9-4) — mac 在线自动更新（绕开签名校验）

- **新增**: mac 自研更新器 — 无 Apple 开发者证书时 electron-updater(Squirrel.Mac) 安装前签名校验必败（"代码不含资源，但签名指示这些资源必须存在"），mac 改为应用内自更新：检查（GitHub Releases latest-mac.yml，按架构选 zip）→ 下载（进度条）→ sha512 校验 → ditto 解压 → 原子替换当前 .app（旧包备份、失败回滚、启动时清理遗留备份）→ 重启；应用自己下载的文件不带 quarantine 属性，替换后不触发 Gatekeeper 二次拦截
- **新增**: 「关于」页更新弹窗新增「在线更新（推荐）」按钮（下载进度 + 重启安装），保留手动下载 dmg 入口；下载中禁止关闭弹窗
- **修复**: 「关于」页手动检查更新指向了上游仓库（chaogei），会导致引导用户去上游下载 — 改为本 fork 仓库
- Windows 更新流程不变（electron-updater NSIS，无需签名）

---

### v1.7.24 (2026-9-4) — 自动刷新指示器文案优化

- **优化**: 自动刷新指示器改为带语义图标 + 人话文案 — 正常态「✓ Token 保活正常 · N 秒/分钟前」；本轮有续期时显示具体动作「刚续期 N 个 Token · N 分钟前」；停滞时明确告警「⚠ 保活没动静了」并建议重启；关闭态说明后果（到期掉线）与开启入口；悬浮提示首行新增一句功能解释，时间显示改为"秒/分钟前"自然语言

---

### v1.7.23 (2026-9-4) — 修复标签菜单颜色不显示

- **修复**: 工具栏标签批量菜单的复选框/部分选中图标颜色不显示或过淡 — 标签颜色为 ARGB（#AARRGGBB）格式，直接写入 CSS 会被按 #RRGGBBAA 解析成高透明度（如已订阅 3%、降 Free 27%），改经 `toRgba` 转换后正常显示

---

### v1.7.22 (2026-9-4) — 账号管理页显示自动刷新实时状态

- **新增**: 账号管理工具栏「自动刷新」实时指示器 — 绿点 + 最近检查时间（主进程刷新池每 60 秒心跳上报），刷新过 token 时显示成功数/总数，超过 3 分钟无心跳变黄提示，设置关闭时显示灰色「自动刷新已关」；悬浮显示 token 保活与信息同步两轮调度的参数与结果
- **新增**: 账号列表行徽章列新增 Token 剩余时间常显（正常灰 / 半小时内橙 / 过期红），自动续期是否保活一眼可见

---

### v1.7.21 (2026-9-4) — 拒卡降级账号识别（managementTarget）

- **增强**: 降级 Free 识别新增 `managementTarget=MANAGE` 判定 — Stripe 订阅系统有记录的 Free 账号（含开 Pro 被银行拒卡后降回 Free 的）不再计入可升级；全新号为 `PURCHASE` 不受影响。旧启发式（willRenew/renewalCheckedAt）保留兜底

---

### v1.7.20 (2026-9-4) — 过滤条件扩展：已用积分的 Free 也排除

- **增强**: 「过滤不适用 Free」开关（原「过滤降级 Free」）除降级账号外，同时排除已使用积分（usage > 0）的 Free 账号 — 非全新账号不适合用来订阅；预检徽章分开展示「降级 Free N」「已用积分 N」

---

### v1.7.19 (2026-9-4) — 可升级列表过滤"降级 Free"

- **新增**: 获取链接 tab 升级预检新增「过滤降级 Free」开关（默认开、跨会话记忆）— 从付费降为 Free 的账号（切 Free / 到期不续费 / 门户侧已降级）不再计入可升级数字与勾选面板，预检徽章单独显示「降级 Free N」
- **标记**: 所有"付费 → Free"转换点写入 `wasPaid` 历史标记，账号刷新永久保留（订阅类型变化也不丢）；存量账号用启发式识别（Free 且带 `willRenew:false` / `renewalCheckedAt` 即为立即切 Free 产物）

---

### v1.7.18 (2026-9-4) — 获取链接支持扫码支付

- **新增**: 链接列表每条新增「二维码」按钮 — 点击弹窗展示该账号支付链接的二维码，手机扫码直接打开 Stripe 支付页面（无需复制链接到手机）；过期链接弹窗内会提示重新生成

---

### v1.7.17 (2026-9-4) — 切 Free 执行范围修复

- **修复**: v1.7.16 只修正了「全部切 Free / 检查全部续费」的按钮数字，实际执行仍会对含已排期在内的全部付费账号提交请求 — 计数与执行现共用同一判定（`isPaidSwitchable`），按钮数字 = 实际处理数

---

### v1.7.16 (2026-9-3) — 切 Free 计数修复 + 获取链接按账号勾选

- **修复**: 「全部切 Free / 检查全部续费」数字把已安排周期末切 Free 的账号也计入（切 Free 周期末生效、订阅名当前仍为 Pro）— 已排期（scheduledToFree）的不再计入
- **新增**: 批量订阅「获取链接」tab 增加账号勾选面板（全选/清空）— 勾选后只对勾选账号获取订阅链接，不勾维持全部可升级账号，按钮数字实时联动

---

### v1.7.15 (2026-9-3) — GitHub 无痕快捷登录 + 注册快捷入口 + Esc 取消选中

#### ⚡ 账号管理快捷入口

- **新增**: 工具栏「GitHub」按钮 — 一键打开添加对话框并自动切到在线登录、强制无痕模式发起 GitHub 授权，全程零点击（不依赖全局隐私模式开关）
- **新增**: 工具栏「注册」按钮 — 无痕模式打开 `app.kiro.dev/signin` 注册页；主进程新增通用 IPC `open-url-private`（仅放行 https 地址）
- **新增**: 账号管理页按 Esc 取消选中全部账号；有对话框打开时不抢按键

---

### v1.7.14 (2026-9-2) — 恢复卡片等高网格

- **调整**: v1.7.13 的动态行高导致同屏卡片高低参差 — 恢复全卡片统一高度；统一高度从 340px 提升到 384px，容纳满配内容（徽章 + 用量 + 明细 + 标签 + 按钮区），标签不再遮挡明细

---

### v1.7.13 (2026-9-2) — 卡片布局修复（标签遮挡）+ 移除来源/已售字段

#### 🐛 账号卡片布局修复

- **修复**: 卡片原固定高度 324px，内容满配（徽章 + 用量 + 额度明细 + 标签 + 底部按钮）总高超出时明细行被压缩、标签行叠在明细文字上 — 改为最小高度 + 虚拟滚动动态测行高，内容少的卡片保持等高，内容多的卡片自动撑开

#### 🗑️ 移除来源/已售字段

- **移除**: 账号的来源标记（注册/导入/手动）与已售标记全链路 — 卡片/列表徽章、筛选面板区块、批量标记按钮、编辑对话框与详情展示、注册时自动标记来源；请统一改用自定义标签维护
- **保留**: 注册元数据（注册时间 / 出口 IP / 指纹版本，封控追溯用）；存量数据中的旧字段不做迁移，仅不再读写

---

### v1.7.12 (2026-9-2) — SQLite 存储与来源/已售标记 + 订阅批量切 Free + CF/GPTmail 取码源 + 主进程 Token 刷新 + 导出格式记忆

> 本段为 v1.7.6–v1.7.12 累积更新（v1.7.6–1.7.9 未单独发版，v1.7.10/1.7.11 发布时更新日志尚未补录）。

#### 💾 账号存储与管理

- **新增**: 账号存储迁移 SQLite 行级 diff — 保存只写变更字段，注册账号自动标记来源并支持按来源筛选
- **新增**: 账号已售标记 — 徽章点击切换、三态筛选（全部/已售/未售）、批量标记，编辑对话框支持来源与售出设置
- **新增**: 账号日期区间筛选 — createdAt 起止 + 今天/近 7 天/近 30 天快捷，筛选变化自动裁剪选中集
- **新增**: 导出对话框记住上次选择的导出格式，切换页面/重启后保持（账号页与设置页共用）
- **修复**: 编辑账号未点「验证并刷新」直接保存，会把基础/试用/奖励额度与重置日期整体清空

#### 🔑 取码源 / 注册

- **新增**: GPTmail（mail.chatgpt.org.uk）域名邮箱取码源 — 私有域名直收与 CF 转发两种模式
- **新增**: CF 自建邮箱取码源（admin 模式）— `/admin/mails` 探活、Pages 误填识别、12 位 base36 混排前缀防撞号
- **修复**: 注册提交邮箱被 AWS WAF 拦截 — TLS 指纹升级 chrome_144
- **修复**: 批量订阅链接 proPlanType 闭包过期，导致取到旧计划
- **修复**: CF 共享 inbox 并发取码改基线 ID 过滤，不再误删其它任务的验证码

#### 📦 订阅 / Stripe

- **新增**: Kiro Pro Max 订阅计划支持
- **新增**: Stripe 门户自动切 Free 与续费状态检查
- **改进**: 批量订阅操作降并发加随机间隔；切 Free 前只读预检，自动跳过不符合条件的账号

#### 🌐 反代 / 稳定性

- **修复**: 工具调用 XML 泄漏跨帧解析并去重，修复工具不执行
- **改进**: token 刷新改单飞（共享在途 Promise）— 等待方拿到真实结果，不再固定等 1s 误判切号
- **改进**: SSE 写缓冲与流解析全程背压，慢客户端不再内存堆积
- **改进**: 429/402 配额耗尽按 quotaResetMs 定时恢复，账号不再被永久跳过
- **新增**: 主进程 token 刷新调度器 — 每 60s 刷新即将过期 token，挂托盘/最小化也照常刷新
- **新增**: 首页告警看板
...
- **新增**: 侧边栏收起状态持久化，重启后保持
- **调整**: macOS 应用图标加圆角
- **修复**: 应用内自动更新源指向本仓库（原指 upstream，安装后会升级成上游版本）
- **修复**: Windows 构建钉住 windows-2022 镜像（windows-latest 的 VS2026 无法被 node-gyp 识别）
