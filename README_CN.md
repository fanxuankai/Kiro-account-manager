...
### v1.7.34 (2026-9-14) — 新增账单页：账单快照 + 汇总卡 + 双维度筛选

- **新增（账单页）**: 侧边栏新增「账单」页 — 顶部四张汇总卡（下期总扣款 / 将续费 / 已排期 Free / 未检查）随筛选实时重算；筛选器与账号管理页同款交互（分组 / 标签 + 账单维度：计划、下期状态）加邮箱昵称搜索；宽表格显示计划单价、计费周期、本周期应收、下期账单金额@日期（红=将扣款、青=$0 已排期 Free）、扣款卡（品牌+末四位，悬浮显示有效期）、最近发票（点击直达 Stripe 官方收据页）；默认按下期金额降序，最烧钱的排最前；「检查选中 / 检查全部账单」走只读门户链路（并发 2）回写快照，行内可单独刷新
- **新增（账单快照，零额外请求）**: 「检查续费 / 切 Free」的 GET subscriptions 响应本就内联计划单价、计费周期、本周期与下期账单金额、扣款卡、最近发票及收据链接 — 此前解析几个 ID 后即丢弃，现在随同一响应回写到 `subscription`（金额为分、时间为毫秒）；GET 追加 `expand=data.default_payment_method` 让卡面信息（品牌/末四位/有效期）内联返回，请求数不变（仍每账号 4 个）
- **范围**: 从未订阅的纯 Free 账号无账单，不出现在账单页；发票历史（完整列表）本期未做，行内只展示最近一张
